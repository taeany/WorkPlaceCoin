...
// Copyright (c) 2016-2018 The WorkPlaceCoin developers
...
public:
    ...
    bool WriteZWOPCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZWOPCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZWOPCSeed_deprecated(uint256& seed);
    bool EraseZWOPCSeed();
    bool EraseZWOPCSeed_deprecated();

    bool WriteZWOPCCount(const uint32_t& nCount);
    bool ReadZWOPCCount(uint32_t& nCount);
