...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::WOPC);
    ...
    if (!settings.contains("nAnonymizeWorkPlaceCoinAmount"))
        settings.setValue("nAnonymizeWorkPlaceCoinAmount", 1000);

    nAnonymizeWorkPlaceCoinAmount = settings.value("nAnonymizeWorkPlaceCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeWorkPlaceCoinAmount"))
        SoftSetArg("-anonymizeworkplacecoinamount", settings.value("nAnonymizeWorkPlaceCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in workplacecoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeWorkPlaceCoinAmount:
            return QVariant(nAnonymizeWorkPlaceCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeWorkPlaceCoinAmount:
            nAnonymizeWorkPlaceCoinAmount = value.toInt();
            settings.setValue("nAnonymizeWorkPlaceCoinAmount", nAnonymizeWorkPlaceCoinAmount);
            emit anonymizeWorkPlaceCoinAmountChanged(nAnonymizeWorkPlaceCoinAmount);
            break;
        case CoinControlFeatures:
