...
#include "ui_zwopccontroldialog.h"
...
std::set<std::string> ZWopcControlDialog::setSelectedMints;
std::set<CMintMeta> ZWopcControlDialog::setMints;

ZWopcControlDialog::ZWopcControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZWopcControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZWopcControlDialog::~ZWopcControlDialog()
{
    delete ui;
}

void ZWopcControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZWopcControlDialog::updateList()
{
...
void ZWopcControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZWopcControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZWopc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZWopcControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZWopcControlDialog::GetSelectedMints()
{
...
void ZWopcControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
