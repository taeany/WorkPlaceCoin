...
#define URI_SCHEME "workplacecoin"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a WorkPlaceCoin address (e.g. %1)").arg("WgYtkT6PiaiezrjQ2ajclJtcfip2SrtaV4"));
...
{
    // return if URI is not valid or is no WorkPlaceCoin: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::WOPC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert workplacecoin:// to workplacecoin:
    //
    //    Cannot handle this later, because workplacecoin:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::WOPC, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open workplacecoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "WorkPlaceCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for WorkPlaceCoin.lnk
...
{
    return GetAutostartDir() / "workplacecoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a workplacecoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=WorkPlaceCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the workplacecoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add workplacecoin app to startup item list
