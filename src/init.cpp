...
#include "config/workplacecoin-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("workplacecoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "workplacecoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "workplacecoind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the WOPC and zWOPC money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in WOPC/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in WOPC/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, workplacecoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in WOPC/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all WorkPlaceCoin specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-wopcstake=<n>", strprintf(_("Enable or disable staking functionality for WOPC inputs (0-1, default: %u)"), 1));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
		   ...
		   "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The WorkPlaceCoin Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("workplacecoin-loadblk");
...
/** Sanity checks
 *  Ensure that WorkPlaceCoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. WorkPlaceCoin is shutting down."));
...
#endif
    // Make sure only a single WorkPlaceCoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. WorkPlaceCoin is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("WorkPlaceCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //WorkPlaceCoin: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //WorkPlaceCoin specific: zerocoin and spork DB's
                ...
                // WorkPlaceCoin: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of WorkPlaceCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart WorkPlaceCoin to complete") << "\n";
        ...
        zwalletMain = new CzWOPCWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zWOPCWallet


        //Load zerocoin mint hashes to memory
        pwalletMain->zwopcTracker->Init();
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeWorkPlaceCoinAmount = 2;
...
//    }
//
//    nAnonymizeWorkPlaceCoinAmount = GetArg("-anonymizeworkplacecoinamount", 0);
//    if (nAnonymizeWorkPlaceCoinAmount > 999999) nAnonymizeWorkPlaceCoinAmount = 999999;
//    if (nAnonymizeWorkPlaceCoinAmount < 2) nAnonymizeWorkPlaceCoinAmount = 2;
    ...
    LogPrintf("Anonymize WorkPlaceCoin Amount %d\n", nAnonymizeWorkPlaceCoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1WOPC +1000 == (.1WOPC +100)*10
       10WOPC +10000 == (1WOPC +1000)*10
    */
    obfuScationDenominations.push_back((5000 * COIN) + 10000000);
    obfuScationDenominations.push_back((5000 * COIN) + 1000000);
