...
#ifndef WorkPlaceCoin_ZWOPCWALLET_H
#define WorkPlaceCoin_ZWOPCWALLET_H
...
public:
    CzWOPCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZWOPC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZWOPC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //WorkPlaceCoin_ZWOPCWALLET_H
