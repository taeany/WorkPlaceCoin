...
CzWOPCWallet::CzWOPCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zwopc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZWOPCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZWOPCSeed_deprecated()) {
                    LogPrintf("%s: Updated zWOPC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zwopc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zwopc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzWOPCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZWOPCCount(nCountLastUsed);
    else if (!walletdb.ReadZWOPCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzWOPCWallet::Lock()
{
    seedMaster = 0;
}

void CzWOPCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzWOPCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZWOPC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzWOPCWallet::LoadMintPoolFromDB()
{
...
void CzWOPCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzWOPCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzWOPCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zwopcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzWOPCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZWOPC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zwopcTracker which also adds to database
    pwalletMain->zwopcTracker->Add(dMint, true);
...
void CzWOPCWallet::SeedToZWOPC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzWOPCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzWOPCWallet::GenerateDeterministicZWOPC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzWOPCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzWOPCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
