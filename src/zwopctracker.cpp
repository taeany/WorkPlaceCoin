...
CzWOPCTracker::CzWOPCTracker(std::string strWalletFile)
{
...
CzWOPCTracker::~CzWOPCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzWOPCTracker::Init()
{
...
bool CzWOPCTracker::Archive(CMintMeta& meta)
{
...
bool CzWOPCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzWOPCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzWOPCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzWOPCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzWOPCTracker::GetSerialHashes()
{
...
CAmount CzWOPCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzWOPCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzWOPCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzWOPCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzWOPCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzWOPCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzWOPCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzWOPCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzWOPCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzWOPCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzWOPCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzWOPCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzWOPCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzWOPCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzWOPCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzWOPCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzWOPCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzwopc from DB\n", __func__, listDeterministicDB.size());
    }
